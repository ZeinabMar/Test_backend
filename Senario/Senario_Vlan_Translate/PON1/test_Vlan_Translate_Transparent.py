...
def test_Vlan_Translate(rest_interface_module, node_id):
 ...
 #**********************************PON Config ****************************************
    ...
    for PORT,information in PORT_INFORMATION.items():
        for info in information :
            response = getall_and_update_condition(rest_interface_module, f"/api/gponconfig/tcont/getall?nodeId={node_id}&shelfId=1&slotId=1&portId={PORT}&onuId=-1")
            Tcont_Management(rest_interface_module, node_id, tcont(1, {"nodeId":None, "slotId":1,"shelfId":1,"bwProfileId":2,"bwProfileName": "HSI", "name": "tcont_valid2", "onuId": info['ONUnumber'], "portId": PORT, "tcontId": 2},
...
# #**********************************PON UNconfig ****************************************
#     uplink_vlan_config(rest_interface_module, node_id, uplink_vlan_conf(1+8, None, "TRUNK", -1, "" , "800,802,900", "", -1, "Pass"), method='POST')
