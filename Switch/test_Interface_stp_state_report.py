...
def Bridge_Stp_report(rest_interface_module, node_id, Bridge_Stp_rp=Bridge_conf(),stpIndex=1, method='POST'):
    ...
    if method == 'GET':  
        ...
        logger.info(f' GETTING Bridge_Stp_report (after {method} method) ... ')
                ...
                #*********************************************************
        if method == 'GET':
            if data.bridgeProtocol == "IEEE" or "IEEE_VLAN_BRIDGE" or "RPVSTP" or "RSTP" or "RSTP_RING" or "RSTP_VLAN_BRIDGE" or "RSTP_VLAN_BRIDGE_RING" or "PROVIDER_RSTP" or "PROVIDER_RSTP_EDGE":
                assert (input_data["stpIndex"] == 1 and
                        # len(str(input_data["specStpBridgeId"])) != 0 and 
                        input_data["specStpBridgePriority"] == 0 and #data.priority
                        ...
                        input_data["specStpStpEnable"] == 1 and
